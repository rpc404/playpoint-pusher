...
const url = process.env.ENV == "development" ? "http://localhost:5173/verify" : "https://playpoint.ai/verify"
module.exports = {

    handleEmail: expressAsyncHandler(
        async(request,response) =>{
            ...
            if(_tokenSent){
                return response.json({msg:"You have just reqested for the OTP. please verify or try after a moment"})
            }
            if(!_email){
                let _exist = true;
                let token;
    ...
    handleToken: expressAsyncHandler(
        async(request, response)=>{
